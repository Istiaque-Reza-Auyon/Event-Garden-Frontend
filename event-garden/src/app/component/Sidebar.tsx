...
const Sidebar:React.FC<SidebarProps> = ({isOpen, toggleMenu}) => {
    ...
    <div className={`fixed inset-0 z-50 transition-transform ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className='flex justify-end'><button className='flex justify-end  p-10 pb-2 text-[rgb(233,186,0)] text-xl'>Explore Events</button></div>
        ...
        <div className='flex justify-end'><button className='flex justify-end p-10 pb-2 text-[rgb(233,186,0)] text-xl'>Launch An Event</button></div>
    </div>
    )
}
