...
const CreateTicket:React.FC<CreateTicketProps> = ({price, toggleMenu}) => {

    const [_, setPrice] = useState(price);
    ...
    return (
        <div className='flex flex-col h-auto w-screen overflow-y-scroll bg-[rgb(5,6,6)] p-3'>
            ...
            <div className='flex'>
                <input className='p-3 pl-2 rounded-lg mb-4 mr-1 w-[50%] bg-[rgb(43,44,44)] text-[rgb(134,135,137)] text-[20px]' name="startTime" type="text" placeholder='Start time*' onFocus={(e) => (e.target.type = "date")} onBlur={(e) => (e.target.type = "text")}   required />
                <input className='p-3 pl-2 rounded-lg mb-4 ml-1 w-[50%] bg-[rgb(43,44,44)] text-[rgb(134,135,137)] text-[20px]' name="endTime" type="text" placeholder='End time*' onFocus={(e) => (e.target.type = "date")} onBlur={(e) => (e.target.type = "text")} required />
            </div>
            ...
            <button className='flex justify-center items-center p-2 bg-[rgb(233,186,0)] text-black mt-5 rounded-[50px]'>Create Ticket</button>
        </div>
    )
}
